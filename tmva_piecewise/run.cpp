...
double get_pot(std::string const & file_path, std::string const & tree_path) {
  ...
  tree_helper th;
  TTree * pot_tree = th.GetTree(file_path, tree_path);
  double pot = -1;
...
int main(int const argc, char const * argv[]) {
  ...
  std::vector<TTree *> const signal_trees = {
    th.GetTree(dir + "/runmv_sp.root", "LEEPhoton/vertex_tree")
  };
  ...
  std::vector<TTree *> const background_trees = {
    th.GetTree(dir + "/runmv_bnb_cosmic.root", "LEEPhoton/vertex_tree")
  };
  ...
  std::vector<std::pair<std::string, std::string>> variables_trackonly = variables;
  ...
  std::vector<std::pair<TTree *, std::string>> trees = {
    std::pair<TTree *, std::string>(th.GetTree(dir + "/runmv_sp.root", "LEEPhoton/vertex_tree"), "signal"),
    std::pair<TTree *, std::string>(th.GetTree(dir + "/runmv_bnb_cosmic.root", "LEEPhoton/vertex_tree"), "background")
  };
  ...
  if(option == "train") {
    train(identifier_notrack, all_cut_notrack, signal_definition, signal_trees, background_trees, variables, methods);
    train(identifier_trackonly, all_cut_trackonly, signal_definition, signal_trees, background_trees, variables_trackonly, methods);
  }
  
  else if(option == "app") {
    app(identifier_notrack, trees, variables, methods);
    app(identifier_trackonly, trees, variables_trackonly, methods);
