...
void train(std::string const & identifier, std::string const & all_cut, std::string const & signal_definition, std::vector<TTree *> const & signal_trees, std::vector<TTree *> const & background_trees, std::vector<std::pair<std::string, std::string>> const & variables, std::vector<method_struct> const & methods, bool const apply_weights = false) {
  ...
  for(TTree * tree : background_trees) {
    ...
    background_entries += tree->GetEntries(all_tcut);
  }

  for(std::pair<std::string, std::string> const & p : variables) dataloader->AddVariable(p.first.c_str());
  
  dataloader->PrepareTrainingAndTestTree(sig_tcut, all_tcut,
