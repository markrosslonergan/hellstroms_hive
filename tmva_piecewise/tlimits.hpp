...
void tlimits_var(std::string const & ifile_path,
  ...
  for(size_t i = 0; i < methods.size(); ++i) {
    std::cout << "Method: " << methods.at(i).str << " CLsexp: " << results.at(i).first << " Signal Vertices: " << oh.GetObject<TH1>(ifile_path, "ncdelta_cosmic_" + methods.at(i).str + "_mva")->Integral() * run_pot / results.at(i).second << " Background Vertices: " << oh.GetObject<TH1>(ifile_path, "bnb_cosmic_background_" + methods.at(i).str + "_mva")->Integral() * run_pot / background_pot << "\n";
  }
