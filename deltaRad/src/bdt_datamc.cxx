...
int bdt_datamc::plotStacks(TFile *ftest, std::vector<bdt_variable> vars, double c1, double c2, bool is_datamc){
    return this->plotStacks(ftest,vars,{c1,c2},is_datamc);
}


int bdt_datamc::plotStacks(TFile *ftest, std::vector<bdt_variable> vars, std::vector<double> bdt_cuts, bool is_datamc){

    //bool is_datamc = false;
    // NEW ONE

    double plot_pot;
    if(is_datamc){
        plot_pot=data_file->pot;
    }else{
        plot_pot = 13.2e20;
    }
    ...
    for(int s = 1; s< bdt_cuts.size()+2; s++){
        ...
        for(auto &var: vars){
            ...
            TPad *pad0top;
            if(is_datamc){
                pad0top =  new TPad(("pad0top_"+stage_names.at(s)).c_str(), ("pad0top_"+stage_names.at(s)).c_str(), 0, 0.35, 1, 1.0);
            } else{
                pad0top = new TPad(("pad0top_"+stage_names.at(s)).c_str(), ("pad0top_"+stage_names.at(s)).c_str(), 0, 0.35, 1, 1.0);
            }
            ...
            pottex.SetNDC();
            std::string pot_draw = data_file->topo_name+" "+to_string_prec(plot_pot/1e20,1)+"e20 POT";
            ...
            if(is_datamc){
                ...
                //tsum->Rebin(data_rebin);
                rat_denom = (TH1*)tsum->Clone(("ratio_denom_"+stage_names.at(s)).c_str());
                for(int i=0; i<rat_denom->GetNbinsX(); i++){
                    rat_denom->SetBinError(i,0.0);
                }	

                ratunit = (TH1*)tsum->Clone(("ratio_unit_"+stage_names.at(s)).c_str());
                ...
                // if(is_datamc){
                ratpre = (TH1*)d0->Clone(("ratio_"+stage_names.at(s)).c_str());
            ...
            std::cout<<"done stage "<<s<<std::endl;
