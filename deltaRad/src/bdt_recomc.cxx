...
int bdt_recomc::plot_recomc(TFile *fout, bdt_file* file, bdt_variable var, double cut_cosmic_val, double cut_bnb_val){
	...
	reco_mc_vec_sel = file->getRecoMCTH1(var, selection, "sel_"+file->tag+"_"+var.name, plot_pot);
	reco_mc_all_sel = (TH1*)file->getTH1(var, selection, "all_sel_"+file->tag+"_"+var.name, plot_pot);
	...
	reco_mc_vec_pre = file->getRecoMCTH1(var, precut , "pre_"+file->tag+"_"+var.name, plot_pot);
	reco_mc_all_pre = (TH1*)file->getTH1(var , precut ,"all_pre_"+file->tag+"_"+var.name, plot_pot);
	 ...
	 **********************************************************************/
		...
		THStack * s_reco_truth_sel = new THStack("Selection",("Selection Total:"+to_string_prec(N_selection,3)).c_str());		
		...
		THStack * s_reco_truth_pre = new THStack("Precuts",("Precuts Total: "+ to_string_prec(N_precuts,3)).c_str());		
		...
		THStack * s_reco_truth_bdt1 = new THStack("Cosmic BDT",("Cosmic BDT: Cut @ "+to_string_prec(cut_cosmic_val,2)+" Total: "+to_string_prec(N_bdt_cosmic,3)).c_str());		
		...
		THStack * s_reco_truth_bdt2 = new THStack("BNB BDT",("BNB: Cut @ "+to_string_prec(cut_bnb_val,2)+ " Total: "+to_string_prec(N_bdt_bnb,3)).c_str());		
