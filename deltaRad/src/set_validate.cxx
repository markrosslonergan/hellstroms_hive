...
int main (int argc, char *argv[]){
    ...
    std::string dir = "/uboone/app/users/markrl/SinglePhotonMCC9_Mar2019/workingdir/MCC9.1/Vertexing_MegaRetreat/";
    ...
    bdt_file *ncdelta_SetA = new bdt_file(dir, "vertexed_ncdelta_SetA.root", "NCDeltaSetA", "hist","singlephoton/",  kBlue-6, bkg_flow);
    bdt_file *ncdelta_SetB = new bdt_file(dir, "vertexed_ncdelta_SetB.root", "NCDeltaSetB", "hist","singlephoton/",  kRed-7, bkg_flow);
    bdt_file *ncdelta_SetC = new bdt_file(dir, "vertexed_ncdelta_SetC.root", "NCDeltaSetC", "hist","singlephoton/",  kGreen-3, bkg_flow);
    bdt_file *ncdelta_SetD = new bdt_file(dir, "vertexed_ncdelta_SetD.root", "NCDeltaSetD", "hist","singlephoton/",  kOrange, bkg_flow);
    bdt_file *ncdelta_SetE = new bdt_file(dir, "vertexed_ncdelta_SetE.root", "NCDeltaSetE", "hist","singlephoton/",  kMagenta, bkg_flow);
    bdt_file *ncdelta_SetF = new bdt_file(dir, "vertexed_ncdelta_SetF.root", "NCDeltaSetF", "hist","singlephoton/",  kCyan, bkg_flow);


//    std::vector<bdt_file*> files = {ncdelta_SetA,ncdelta_SetB,ncdelta_SetC,ncdelta_SetD,ncdelta_SetE,ncdelta_SetF};
    std::vector<bdt_file*> files = {ncdelta_SetA,ncdelta_SetB,ncdelta_SetC,ncdelta_SetE,ncdelta_SetF};
    for(auto &f: files){
        f->calcPOT();
    }
    ...
    compareQuick({v_vertexMult,v_vertexMult,v_vertexMult,v_vertexMult,v_vertexMult},files, {"1","1","1","1","1"}, "vertex_multiplicity");

    bdt_variable v_showerMult("reco_asso_showers","(5,0,5)","Number of Showers in Neutrino Slice","false","d");
    compareQuick({v_showerMult,v_showerMult,v_showerMult,v_showerMult,v_showerMult},files, {"1","1","1","1","1"}, "shower_multiplicity");

    bdt_variable v_trackMult("reco_asso_tracks","(5,0,5)","Number of Tracks in Neutrino Slice","false","d");
    compareQuick({v_trackMult,v_trackMult,v_trackMult,v_trackMult,v_trackMult},files, {"1","1","1","1","1"}, "track_multiplicity");

    bdt_variable v_pfpMult("reco_asso_tracks+reco_asso_showers","(8,0,8)","Number of PFP in Neutrino Slice","false","d");
    compareQuick({v_pfpMult,v_pfpMult,v_pfpMult,v_pfpMult,v_pfpMult},files, {"1","1","1","1","1"}, "pfp_multiplicity");


    bdt_variable v_sigMult("(reco_asso_tracks==1 && reco_asso_showers==1)","(4,0,4)","Did we get 1 track and 1 shower?","false","d");
    compareQuick({v_sigMult,v_sigMult,v_sigMult,v_sigMult,v_sigMult},files, {signal_definition,signal_definition,signal_definition,signal_definition,signal_definition}, "sig_multiplicity",false);

    bdt_variable v_sigWrong1Mult("(reco_asso_tracks!=1 && reco_asso_showers==1)","(4,0,4)","Did we get !1 track and 1 shower?","false","d");
    compareQuick({v_sigWrong1Mult,v_sigWrong1Mult,v_sigWrong1Mult,v_sigWrong1Mult,v_sigWrong1Mult},files, {signal_definition,signal_definition,signal_definition,signal_definition,signal_definition}, "sig_wrong1_multiplicity",false);

    bdt_variable v_sigWrong2Mult("(reco_asso_tracks==1 && reco_asso_showers!=1)","(4,0,4)","Did we get 1 track and !1 shower?","false","d");
    compareQuick({v_sigWrong2Mult,v_sigWrong2Mult,v_sigWrong2Mult,v_sigWrong2Mult,v_sigWrong2Mult},files, {signal_definition,signal_definition,signal_definition,signal_definition,signal_definition}, "sig_wrong2_multiplicity",false);



    for(auto &f:files){

        double n_all = f->GetEntries(signal_definition);
        double n_sig = f->GetEntries("reco_asso_tracks==1 && reco_asso_showers==1 && "+signal_definition);

        std::cout<<f->tag<<" has "<<n_sig/n_all*100.0<<" percent of signal 1g1p"<<std::endl;
    }
    ...
    bdt_variable v_showerCorrect("(reco_asso_showers==1 && sim_shower_pdg==22)","(4,0,4)","Did we get 1 Shower matched to a photon?","false","d");
    compareQuick({v_showerCorrect,v_showerCorrect,v_showerCorrect,v_showerCorrect,v_showerCorrect},files, {signal_definition,signal_definition,signal_definition,signal_definition,signal_definition}, "shower_correct",false);

    bdt_variable v_trackCorrect("(reco_asso_tracks==1 && sim_track_pdg==2212)","(4,0,4)","Did we get 1 Track matched to a proton?","false","d");
    compareQuick({v_trackCorrect,v_trackCorrect,v_trackCorrect,v_trackCorrect,v_trackCorrect},files, {signal_definition,signal_definition,signal_definition,signal_definition,signal_definition}, "track_correct",false);


    std::string s_reco_vertex_res_Z ="fabs(reco_vertex_z - mctruth_nu_vertex_z)";
    bdt_variable v_vertexResZ (s_reco_vertex_res_Z,"(100,0,25)","vertex resolution Z cm", false,"d");
    v_vertexResZ.is_logplot=true;
    compareQuick({v_vertexResZ,v_vertexResZ,v_vertexResZ,v_vertexResZ,v_vertexResZ},files, {signal_definition,signal_definition,signal_definition,signal_definition,signal_definition}, "vertex_resZ",false);
    ...
    bdt_variable v_convDist(s_reco_conv_dist,"(100,0,50)","Conv Dist cm", false,"d");
    v_convDist.is_logplot=true;
    std::string add0 = "&&reco_asso_tracks==1 && reco_asso_showers==1";
    compareQuick({v_convDist,v_convDist,v_convDist,v_convDist,v_convDist},files, {signal_definition+add0,signal_definition+add0,signal_definition+add0,signal_definition+add0,signal_definition+add0}, "ConvDist",true);


    bdt_variable v_trackVertexDist( "sqrt(pow(reco_track_startx - reco_vertex_x,2)-pow(reco_track_starty - reco_vertex_y,2)-pow(reco_track_startz - reco_vertex_z,2))"  ,"(20,0,5)","Track Start Vertex Dist cm", false,"d");
    compareQuick({v_trackVertexDist,v_trackVertexDist,v_trackVertexDist,v_trackVertexDist,v_trackVertexDist},files, {signal_definition+add0,signal_definition+add0,signal_definition+add0,signal_definition+add0,signal_definition+add0}, "TrackVertexDist",true);

    bdt_variable v_trackLength("reco_track_displacement", "(20,0,20)","Track Displacement [cm]",false,"d");
    compareQuick({v_trackLength,v_trackLength,v_trackLength,v_trackLength,v_trackLength},files, {signal_definition+add0,signal_definition+add0,signal_definition+add0,signal_definition+add0,signal_definition+add0}, "TrackLength",false);
    ...
    std::string highE_s = "reco_shower_energy_max[0] > 0.2 && mctruth_delta_proton_energy-0.938272 > 0.15 && mctruth_nu_vertex_z < 500";

    //And now for an A <-> E comparason
    std::cout<<"Different! SetA"<<std::endl;
    ncdelta_SetA->tvertex->Scan("run_number:subrun_number:event_number",(signal_definition+"&& reco_asso_tracks==0 && reco_asso_showers==1 &&"+highE_s).c_str())   ;
        
    std::cout<<"Different! SetE"<<std::endl;
    ncdelta_SetE->tvertex->Scan("run_number:subrun_number:event_number",(signal_definition+"&& reco_asso_tracks==1 && reco_asso_showers==1 &&"+highE_s).c_str())  ; 
