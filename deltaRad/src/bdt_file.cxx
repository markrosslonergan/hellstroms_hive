    ...
    is_mc("true")
{
	...
	for(int i=0; i<tpot->GetEntries(); i++){
	    ...
	    tmppot += potbranch; //Extract pot # for each entry
	}
	numberofevents = tmpnum;
	pot=tmppot;
	std::cout<<"--> POT is MC: Row";
	...
	if (tag != "NueIntrinsicCosmics"){
	    weight_branch = "(lee_signal_weights.lee_weights)";
	}
	//--> POT is MC: --> value: 05.72623e+22 NumEvents: 592200
	//BETTER USE WES's result? 1e20/206.2*numberofevents;
	
	//pot = 1e20*numberofevents/206.2;//5.72623e22*(double)numberofevents/592200.0;
	pot = 4.9545455e22; //Calculate it by comparing events numebrs and 
			    // Wes' result within a fiducial volumne
	std::cout<<"\n(Done manually) REAL MCC9: --> POT is MC: ";
	std::cout<<"--> value: "<<pot<<" NumEvents: "<<numberofevents<<std::endl;
    }
