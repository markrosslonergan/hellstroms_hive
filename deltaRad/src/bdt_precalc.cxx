...
int bdt_precalc::genNewTrackInfo(){
	...
	TTree * friend_tree = new TTree("newtrack_info","newtrack_info");
	...
	std::vector<std::vector<double>> *vtrk_energy_from_dEdx= 0;
	std::vector<std::vector<double>> *vtrk_energy_new = 0;

	TBranch *bdEdx = 0;
	TBranch *bresrange = 0;
	TBranch *btrackpdg = 0;

	TBranch *benergy_new = 0;
	TBranch *benergy_from_dEdx = 0;

	file->tvertex->SetBranchAddress("reco_asso_tracks", &reco_asso_tracks);
	file->tvertex->SetBranchAddress("reco_asso_showers", &reco_asso_showers);
	file->tvertex->SetBranchAddress("reco_track_calo_dEdxnew",&vtrk_dEdx,&bdEdx);
	
	file->tvertex->SetBranchAddress("reco_track_energy_new",&vtrk_energy_new,&benergy_new);
	file->tvertex->SetBranchAddress("reco_track_energy_from_dEdx",&vtrk_energy_from_dEdx,&benergy_from_dEdx);
	...
	std::vector<double> v_track_energy_from_dEdx;
	std::vector<double> v_track_energy_new;
	...
	for(int i=0; i< file->tvertex->GetEntries(); i++){
		...
		if(reco_asso_tracks == 0){
			...
			for(int s=0; s<reco_asso_tracks; s++){
				...
				if(vtrk_resrange->at(s).size() != vtrk_dEdx->at(s).size()){
					std::cout<<"resrange and dEdx not the same vector! "<<	vtrk_resrange->at(s).size()<<" "<<vtrk_dEdx->at(s).size()<<std::endl;
					exit(EXIT_FAILURE);
				}
