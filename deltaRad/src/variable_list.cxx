...
variable_list::variable_list(std::string analysis_tag_in): analysis_tag(analysis_tag_in){
	...
	if(analysis_tag == "track" || analysis_tag == "notrack"){
		...
		if(analysis_tag == "track"){
			...
			all_vars.push_back(bdt_variable("reco_track_mean_trunc_dEdx", "(48,0,12)","Mean Track dE/dx [MeV/cm]", true,"d"));//23
			all_vars.push_back(bdt_variable("reco_track_mean_trunc_dEdx_start_half[0]/reco_track_mean_trunc_dEdx_end_half", "(48,0,3.5)","Ratio of Mean Start/End Track dE/dx", true,"d"));//24
			...
			all_vars.push_back(bdt_variable("cos(reco_track_theta_yz)","(48,-1,1)","Reconstructed Track - Cosine Theta", true,"d"));//29
			all_vars.push_back(bdt_variable("cos(reco_track_phi_yx)","(48,-1,1)","Reconstructed Track - Cosine Phi", true,"d"));//30
