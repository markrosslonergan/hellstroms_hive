...
//    std::string analysis_tag ="electron0";//for 1e0p event
    ...
    std::string temp_tag = analysis_tag;
     
    if(!run_bnb){//do cosmic
	temp_tag = analysis_tag + "cosmic";
    }
    else if(!run_cosmic){//do bnb
	temp_tag = analysis_tag + "bnb";
    } //do everything if both run_bnb and cosmic_bnb are true.
    ...
    }else if (analysis_tag == "electron0") {

	num_track_cut = "== 0";
	bnb_bdt_info.setTopoName("1e0p");
	cosmic_bdt_info.setTopoName("1e0p");
    }else if (analysis_tag == "electron2") {

	num_track_cut = "== 2";
	bnb_bdt_info.setTopoName("1e2p");
	cosmic_bdt_info.setTopoName("1e2p");
    }
...
//CHECK Add a list for -n options of files.	
	if(number == -1){
 ...
 */
	...
	histogram_stack.addToStack(intrinsics);
	...
	//TO BE FIXED
	bdt_stack *cosmic_stack = new bdt_stack(analysis_tag+"_datamc");
	cosmic_stack->plot_pot = 4.898e19;
...
/*
	...
	std::cout<<"\\hline & \\multicolumn{2}{c}{LEE $\\nu_e$ w/Cosmics} &              \\multicolumn{2}{c}{BNB w/Cosmic}  & \\multicolumn{2}{c}{BNB External Cosmics}  \\\\"<<std::endl;
	std::cout<<"\\hline"<<std::endl;
	std::cout<<"Stage & Events & Efficiency & Events & Efficiency & Events &         Efficiency\\\\"<<std::endl;
	std::cout<<"\\hline"<<std::endl;
