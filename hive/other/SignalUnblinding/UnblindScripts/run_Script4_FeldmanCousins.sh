...
if (! $RUND); then
    ...
    ./sbnfit_uboone_scaling_fc -t $TAG -x $XML -i NCDelta -g '1e-6 8.0 50' -n 1000  --mode feldman --cnp &>LOG.feldman.long
    ./sbnfit_uboone_scaling_fc -t $TAG -x $XML -i NCDelta -g '1e-6 8.0 50' -n 1000  --mode belt --cnp &> LOG.belt.long
...
if ($RUND); then
    ...
    ./sbnfit_uboone_scaling_fc -t $TAG -x $XML -i NCDelta -g '1e-6 8.0 50' -n 1000  --mode data --cnp -d "Data_"$TAG"_CV.SBNspec.root" &> LOG.FC.Data.run
fi
