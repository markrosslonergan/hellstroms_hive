...
void bdt_file::MakeFlatTree(const std::vector<std::pair<std::string, int>>& variables, const std::string& treename, const std::string& num_candidate_var){
    ...
    TFile *f = new TFile(flat_filename.c_str(),"update");
    f->cd();
    ...
    for(size_t i=0; i< tvertex->GetEntries(); ++i){
        ...
        if(i%1000==0) std::cout<<i<<"/"<<tvertex->GetEntries()<< std::endl;
