...
bool VRAnalyzer::Run() {
  ...
  if(frun_pandora) {
    ...
    std::cout<<" VRAnalyzer::Run() || Running in Vertex Builder Mode"<<std::endl;
    AddTracks(pas.GetDetectorObjects(), ftrack_producer);
    AddShowers(pas.GetDetectorObjects(), fshower_producer);
    vb.Run(pas);
  }
